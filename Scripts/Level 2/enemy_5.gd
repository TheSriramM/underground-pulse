...
# The third enemy and the fifth enemy share a area2d
func _on_enemy_3_area_2_area_entered(area: Area2D) -> void:
...
func _on_enemy_5_area_2_area_entered(area: Area2D) -> void:
	if area.get_parent() == self:
		right = true
		$fov.rotation_degrees = -90
